...
require_once(PHPC_ROOT_PATH . '/src/display_functions.php');
...
class MonthPage extends Page
{
	...
	{
		$cid = $context->calendar->cid;
		$month = $context->month;
		$year = $context->year;

		$months = array();
		for($i = 1; $i <= 12; $i++) {
			$months[PHPC_HOME_URL . "?action=display_month&amp;phpcid=$cid&amp;month=$i&amp;year=$year"] =
				month_name($i);
		}
		$years = array();
		for($i = $year - 5; $i <= $year + 5; $i++) {
			$years[PHPC_HOME_URL . "?action=display_month&amp;phpcid=$cid&amp;month=$month&amp;year=$i"] = $i;
		}
		$next_month = $month + 1;
		$next_year = $year;
		if($next_month > 12) {
		...
		$heading = tag('',
				tag('a', attrs('class="phpc-icon-link"',
						'href="' . PHPC_HOME_URL . "?action=display_month&amp;phpcid=$cid&amp;month=$prev_month&amp;year=$prev_year\""),
					tag('span', attrs('class="fa fa-chevron-left"'), '')),
				create_dropdown_list(month_name($month), $months),
				create_dropdown_list($year, $years),
				tag('a', attrs('class="phpc-icon-link"',
						'href="' . PHPC_HOME_URL . "?action=display_month&amp;phpcid=$cid&amp;month=$next_month&amp;year=$next_year\""),
